...
def validate_target(value: str) -> dict:
    ...
    try:
        ip = ipaddress.ip_address(host); is_ip = True
    ...
    if blocked: raise HTTPException(status_code=400, detail="Private, loopback, link-local, multicast, reserved, or local targets are not allowed")
...
async def resolve_host(host: str):
    ...
    except socket.gaierror as exc: raise HTTPException(status_code=404, detail=f"DNS resolution failed: {exc}")
...
app.include_router(auth_router); app.include_router(scans_router); app.include_router(reports_router); app.include_router(events_router); app.include_router(findings_router); app.include_router(investigations_router); app.include_router(audit_router)
