...
def issue_token(actor:str,role:str,workspace_id:str,user_id:str|None=None,hours:int|None=None)->str:
    expires=int((datetime.now(timezone.utc)+timedelta(hours=hours if hours is not None else settings.SESSION_HOURS)).timestamp());uid=user_id or "";payload=f"{actor}|{role}|{workspace_id}|{uid}|{expires}";return f"{payload}|{_sign(payload)}"
def principal_from_token(token:str)->Principal:
    ...
    if not user_id:raise HTTPException(401,"Authentication token missing user identity")
...
async def current_principal(authorization:str|None=Header(default=None))->Principal:
    ...
    async with SessionLocal() as db:
        ...
        member=await db.scalar(select(WorkspaceMember).where(WorkspaceMember.user_id==principal.user_id,WorkspaceMember.workspace_id==principal.workspace_id))
    if not user or not member:raise HTTPException(401,"Session is no longer active")
...
async def issue_ws_ticket(principal:Principal,scan_id:str)->str:
    nonce=secrets.token_urlsafe(24);exp=int((datetime.now(timezone.utc)+timedelta(seconds=WS_TICKET_TTL_SECONDS)).timestamp());payload={"purpose":"scan.websocket","scan_id":scan_id,"workspace_id":principal.workspace_id,"user_id":principal.user_id,"actor":principal.actor,"role":principal.role,"exp":exp,"nonce":nonce}
    client=Redis.from_url(os.getenv("PHANTOM_REDIS_URL","redis://localhost:6379/0"),decode_responses=True)
    try:created=await client.set(f"phantom:ws-ticket:{nonce}","1",ex=WS_TICKET_TTL_SECONDS,nx=True)
    finally:await client.aclose()
    if not created:raise HTTPException(503,"Unable to issue WebSocket ticket")
...
async def consume_ws_ticket(token:str,expected_scan_id:str)->Principal:
    ...
    return Principal(str(payload.get("actor","")),str(payload.get("role","")),str(payload.get("workspace_id","")),str(payload.get("user_id","")) or None)
