...
def _digest(v:str)->str:return hashlib.sha256(v.encode()).hexdigest()
def _sign(payload:str)->str:return hmac.new(settings.AUTH_SECRET.encode(),payload.encode(),hashlib.sha256).hexdigest()
def issue_token(actor:str,role:str,workspace_id:str,user_id:str|None=None)->str:
    expires=int((datetime.now(timezone.utc)+timedelta(hours=settings.SESSION_HOURS)).timestamp()); uid=user_id or ""
