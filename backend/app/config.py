...
class Settings:
    ...
    # Defaults to the existing Railway Postgres service with an isolated auth schema.
    # A separate Railway Postgres service can be supplied later with PHANTOM_AUTH_DATABASE_URL.
    AUTH_DATABASE_URL=_normalize_database_url(os.getenv("PHANTOM_AUTH_DATABASE_URL","")) or DATABASE_URL
    ...
    SCAN_MODULE_TIMEOUT_SECONDS=max(5,int(os.getenv("PHANTOM_SCAN_MODULE_TIMEOUT_SECONDS","120")))
    SCAN_TOTAL_TIMEOUT_SECONDS=max(30,int(os.getenv("PHANTOM_SCAN_TOTAL_TIMEOUT_SECONDS","900")))
    ...
    SCAN_REQUEST_BUDGET=max(1,int(os.getenv("PHANTOM_SCAN_REQUEST_BUDGET","250")))
    SCAN_MAX_REDIRECTS=max(0,int(os.getenv("PHANTOM_SCAN_MAX_REDIRECTS","3")))
    SCAN_HTTP_TIMEOUT_SECONDS=max(1,int(os.getenv("PHANTOM_SCAN_HTTP_TIMEOUT_SECONDS","8")))
    SCAN_CONNECT_TIMEOUT_SECONDS=max(1,int(os.getenv("PHANTOM_SCAN_CONNECT_TIMEOUT_SECONDS","5")))
    SCAN_MAX_RESPONSE_BYTES=max(65536,int(os.getenv("PHANTOM_SCAN_MAX_RESPONSE_BYTES","2097152")))
