...
class PasswordChangeRequest(BaseModel):
    current_password:str=Field(min_length=8,max_length=256); new_password:str=Field(min_length=8,max_length=256)
class PasswordResetRequest(BaseModel):
    email:str=Field(min_length=3,max_length=255)
class PasswordResetConfirm(BaseModel):
    token:str=Field(min_length=20,max_length=512); new_password:str=Field(min_length=8,max_length=256)
class VerifyEmailRequest(BaseModel):
    token:str=Field(min_length=20,max_length=512)
class InvitationAccept(BaseModel):
    token:str=Field(min_length=20,max_length=512); password:str=Field(min_length=8,max_length=256)
...
async def login(payload:LoginRequest,request:Request,db:AsyncSession=Depends(get_db)):
    ...
    if not valid:
        ...
        if user.failed_login_count>=MAX_FAILURES:
            user.locked_until=now+timedelta(minutes=LOCK_MINUTES); user.failed_login_count=0
        await record_audit(db,request,"auth.login_failed","user",user.id,{"email":email},actor=email)
        await db.commit()
        raise HTTPException(401,"Invalid credentials")
    if user.mfa_enabled and (not payload.mfa_code or not verify_totp(user.mfa_secret or "", payload.mfa_code)):
        await record_audit(db,request,"auth.mfa_failed","user",user.id,None,actor=email)
        await db.commit()
    ...
    response=await _session_response(user,member,db,request)
    await record_audit(db,request,"auth.login","auth_session",response.user_id,{"workspace_id":member.workspace_id},actor=user.email)
    await db.commit()
    return response
...
async def refresh(payload:RefreshRequest,request:Request,db:AsyncSession=Depends(get_db)):
    ...
    user=await db.scalar(select(User).where(User.id==s.user_id,User.is_active.is_(True)))
    member=await db.scalar(select(WorkspaceMember).where(WorkspaceMember.user_id==s.user_id,WorkspaceMember.workspace_id==s.workspace_id))
    if not user or not member:
        s.revoked_at=_now(); await db.commit(); raise HTTPException(401,"Session is no longer active")
    s.revoked_at=_now()
    response=await _session_response(user,member,db,request)
    await record_audit(db,request,"auth.session_refreshed","auth_session",s.id,None,actor=user.email)
    await db.commit()
    return response

@router.get("/me")
async def me(principal: Principal = Depends(current_principal)):
    from ..rbac import ALL_PERMISSIONS, role_permissions
    role = Role(principal.role.lower())
    permissions = role_permissions(role.value)
    matrix = {role_name.value: sorted(role_permissions(role_name.value)) for role_name in Role}
    return {
...
async def logout(principal:Principal=Depends(current_principal),request:Request=None,db:AsyncSession=Depends(get_db)):
    ...
    if s and not s.revoked_at:
        s.revoked_at=_now()
        await record_audit(db,request,"auth.logout","auth_session",s.id,None,principal)
    await db.commit()
    return {"revoked":True}
...
async def revoke_all(principal:Principal=Depends(current_principal),request:Request=None,db:AsyncSession=Depends(get_db)):
    rows=await db.scalars(select(AuthSession).where(AuthSession.user_id==principal.user_id,AuthSession.revoked_at.is_(None)))
    now=_now(); count=0
    for s in rows.all(): s.revoked_at=now; count+=1
    await record_audit(db,request,"auth.sessions_revoked_all","user",principal.user_id,{"count":count},principal)
    await db.commit()
...
async def reset_request(payload:PasswordResetRequest,request:Request,db:AsyncSession=Depends(get_db)):
    ...
    if user:
        db.add(PasswordResetToken(user_id=user.id,token_hash=_hash_token(token),expires_at=_now()+timedelta(minutes=30)))
        await record_audit(db,request,"auth.password_reset_requested","user",user.id,None,actor=email)
    await db.commit()
...
async def reset_confirm(payload:PasswordResetConfirm,request:Request,db:AsyncSession=Depends(get_db)):
    ...
    for s in rows.all(): s.revoked_at=_now()
    await record_audit(db,request,"auth.password_reset_completed","user",user.id,None,actor=user.email)
    await db.commit()
    return {"reset":True}
...
async def mfa_setup(principal:Principal=Depends(current_principal),db:AsyncSession=Depends(get_db)):
    user=await db.get(User,principal.user_id)
    secret=user.mfa_secret or new_totp_secret()
...
async def mfa_enable(code:str,principal:Principal=Depends(current_principal),request:Request=None,db:AsyncSession=Depends(get_db)):
    ...
    await record_audit(db,request,"auth.mfa_enabled","user",user.id,None,principal)
    await db.commit()
    return {"enabled":True}
...
async def mfa_disable(code:str,principal:Principal=Depends(current_principal),request:Request=None,db:AsyncSession=Depends(get_db)):
    ...
    await record_audit(db,request,"auth.mfa_disabled","user",user.id,None,principal)
    await db.commit()
...
async def accept_invitation(payload:InvitationAccept,request:Request,db:AsyncSession=Depends(get_db)):
    ...
    if user and await db.scalar(select(WorkspaceMember).where(WorkspaceMember.user_id==user.id,WorkspaceMember.workspace_id==row.workspace_id)):
        raise HTTPException(409,"Invitation has already been accepted")
    if not user:
        new_hash=hash_password(payload.password)
        user=User(email=row.email,display_name=row.display_name,password_hash=new_hash,email_verified=False,is_active=True)
...
async def verify_email(payload:VerifyEmailRequest,request:Request,db:AsyncSession=Depends(get_db)):
    ...
    user=await db.get(User,row.user_id)
    if not user: raise HTTPException(404,"User not found")
    user.email_verified=True; row.used_at=_now()
    await record_audit(db,request,"auth.email_verified","user",user.id,None,actor=user.email)
    await db.commit()
