...
def serialize(f: Finding) -> dict:
    return {
        ...
        "cve": f.cve, "cwe": f.cwe, "cvss": f.cvss, "assignee": f.assignee,
...
async def update_finding(finding_id: str, update: FindingUpdate, db: AsyncSession = Depends(get_db)):
    finding = await db.get(Finding, finding_id)
    if not finding: raise HTTPException(404, "Finding not found")
    if update.status is not None:
        if update.status not in STATUSES: raise HTTPException(400, "Invalid finding status")
        if update.status != finding.status and update.status not in TRANSITIONS.get(finding.status, set()):
            raise HTTPException(409, f"Invalid lifecycle transition: {finding.status} -> {update.status}")
        finding.status = update.status
    if update.assignee is not None: finding.assignee = update.assignee.strip() or None
