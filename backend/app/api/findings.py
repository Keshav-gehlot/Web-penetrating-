...
def serialize(f):
    return {
        ...
        "product": getattr(f, "product", None), "version": getattr(f, "version", None), "cpe": getattr(f, "cpe", None),
        "cvss_v3": {"score": getattr(f, "cvss_v3_score", None), "vector": getattr(f, "cvss_v3_vector", None), "severity": getattr(f, "cvss_v3_severity", None)} if getattr(f, "cvss_v3_score", None) is not None else None,
        "cvss_v4": {"score": getattr(f, "cvss_v4_score", None), "vector": getattr(f, "cvss_v4_vector", None), "severity": getattr(f, "cvss_v4_severity", None)} if getattr(f, "cvss_v4_score", None) is not None else None,
        "cve_published_at": getattr(f, "cve_published_at", None).isoformat() if getattr(f, "cve_published_at", None) else None,
        "cve_modified_at": getattr(f, "cve_modified_at", None).isoformat() if getattr(f, "cve_modified_at", None) else None,
        "affected_versions": getattr(f, "cve_affected_versions", []) or [], "references": getattr(f, "cve_references", []) or [],
