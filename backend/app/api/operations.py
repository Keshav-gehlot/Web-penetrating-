...
async def events(limit:int=50,principal:Principal=Depends(require_permission("scan:view")),db=Depends(get_db)):
    ...
    return [{"id":e.id,"event_type":e.event_type,"severity":e.severity,"message":e.message,"scan_id":e.scan_id,"metadata":e.metadata,"created_at":e.created_at.isoformat() if e.created_at else None} for e in rows.all()]
