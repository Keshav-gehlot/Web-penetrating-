...
async def record_audit(db:AsyncSession,request:Request,action:str,resource_type:str,resource_id:str|None=None,metadata:dict[str,Any]|None=None,principal:Principal|None=None):
    actor=principal.actor if principal else request.headers.get("X-PHANTOM-Actor","unknown")
    event=AuditEvent(actor=actor,action=action,resource_type=resource_type,resource_id=resource_id,metadata_json=metadata or {})
...
async def list_audit(db:AsyncSession=Depends(get_db),limit:int=100):
    limit=max(1,min(limit,250)); rows=await db.scalars(select(AuditEvent).order_by(AuditEvent.created_at.desc()).limit(limit))
