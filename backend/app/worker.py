...
async def claim_scan(scan_id: str, attempt: int) -> bool:
    ...
    async with SessionLocal() as db:
        result = await db.execute(
            ...
            .values(
                status="running", started_at=now, worker_id=CONSUMER,
                lease_expires_at=lease, cancel_requested_at=None,
                cancelled_at=None, error=None, attempt=attempt,
            )
        )
        await db.commit()
...
async def process_message(client, message_id: str, fields: dict[str, str]) -> None:
        ...
        while not heartbeat_stop.is_set():
            try:
                ...
                if not await refresh_lease(scan_id):
                    heartbeat_stop.set()
                    return
                if not await refresh_slot(client, slot_key, slot_token):
                    heartbeat_stop.set()
                    return
    ...
    try:
        ...
        await retry_or_fail(
            client, message_id, scan_id, attempt,
            f"Scan exceeded {settings.SCAN_TOTAL_TIMEOUT_SECONDS}s execution budget",
        )
...
async def recover_stale_jobs(client) -> None:
    ...
    while True:
        result = await client.xautoclaim(
            SCAN_STREAM, SCAN_GROUP, CONSUMER,
            JOB_LEASE_SECONDS * 1000, cursor, count=50,
        )
...
async def main() -> None:
    ...
    log.info(
        ...
        SCAN_STREAM, SCAN_GROUP, CONSUMER, settings.MAX_CONCURRENT_SCANS,
    )
    try:
        while True:
            now = time.time()
            ...
            if now - last_recovery >= 10:
                ...
                if recovered:
                    log.warning("Recovered %s stale scan(s)", recovered)
                last_recovery = now
