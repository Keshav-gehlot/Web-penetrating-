...
export type ScanEvent={event:'connected'|'scan.created'|'scan.started'|'scan.completed'|'scan.cancelled';scan_id:string}|{event:'scan.failed';scan_id:string;error:string}|{event:'module.started'|'module.completed';scan_id:string;module:string;index:number;total:number;finding_count?:number}|{event:'finding.created';scan_id:string;finding:FindingEvent};
