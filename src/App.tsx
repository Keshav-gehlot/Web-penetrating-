...
import { RBACProvider, useRBAC } from './lib/RBACProvider';
...
function Protected() {
  ...
  useEffect(() => {
    const timer = window.setInterval(() => {
      ...
      if (!current) return;
      const exp = current.expires_at ? Date.parse(current.expires_at) : 0;
      if (exp && exp - Date.now() < 60000) void refreshSession();
    }, 30000);
  ...
  if (loading) {
    return <div className="h-screen w-full bg-phantom-bg text-phantom-text-secondary flex items-center justify-center text-sm">Loading authorization policy…</div>;
  }

  // This guard improves navigation UX only. Every API operation remains
  // protected by the backend permission checks.
      ...
      <main className="flex-1 flex flex-col min-w-0 h-full relative overflow-hidden bg-phantom-bg rounded-l-xl border border-phantom-border ring-1 ring-white/5 my-2 mr-2 shadow-2xl">
        <div className="flex-1 overflow-y-auto w-full h-full">
          <Routes>
            ...
            <Route path="/netwatch" element={<NetWatch />} />
...
export default function App() {
  const location = useLocation();
  if (location.pathname === '/invite/accept') return <AcceptInvitation />;
  if (location.pathname === '/login') {
    if (getSession()) return <Navigate to="/dashboard" replace />;
    return <Routes><Route path="/login" element={<Login />} /></Routes>;
  }
  return <RBACProvider><Protected /></RBACProvider>;
}
