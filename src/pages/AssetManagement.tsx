...
export default function AssetManagement() {
  ...
  return <div className="flex flex-col h-full bg-phantom-bg">
    <header className="flex-none border-b border-phantom-border bg-phantom-surface/70 px-6 py-5">
      ...
      <div className="flex flex-wrap items-center gap-2 mt-4"><button onClick={importInventory} disabled={!can('workspace:manage')} title={!can('workspace:manage') ? 'You do not have permission to import assets' : 'Import CSV'} className="h-9 px-3 border border-phantom-border rounded-lg text-xs">Import CSV</button><button onClick={()=>void exportInventory()} className="h-9 px-3 border border-phantom-border rounded-lg text-xs">Export CSV</button>{selectedIds.length>0&&<><button onClick={()=>void bulk("activate")} disabled={!can('workspace:manage')} className="h-9 px-3 border border-phantom-border rounded-lg text-xs">Activate ({selectedIds.length})</button><button onClick={()=>void bulk("deactivate")} disabled={!can('workspace:manage')} className="h-9 px-3 border border-phantom-border rounded-lg text-xs">Deactivate</button><button onClick={()=>void bulk("delete")} disabled={!can('workspace:manage')} className="h-9 px-3 border border-phantom-coral/30 text-phantom-coral rounded-lg text-xs">Delete</button></>}</div><div className="flex flex-wrap items-center gap-2 mt-2">
        ...
        <table className="w-full text-left">
          ...
          <tbody>
            ...
            assets.map(a => <tr key={a.id} className="border-b border-phantom-border last:border-0 hover:bg-phantom-panel-hover">
              ...
              <td className="px-4 py-3"><div className="flex justify-end gap-1"><button title={!can('workspace:manage') ? "You do not have permission to edit assets" : "Edit asset"} onClick={() => openEdit(a)} disabled={!can('workspace:manage')} className="p-2 text-phantom-text-tertiary hover:text-phantom-cyan"><Pencil size={14}/></button><button title={!can('workspace:manage') ? "You do not have permission to delete assets" : "Delete asset"} onClick={() => void remove(a)} disabled={!can('workspace:manage')} className="p-2 text-phantom-text-tertiary hover:text-phantom-coral"><Trash2 size={14}/></button></div></td>
            </tr>)}
          </tbody>
        </table>
      </div>
    ...
    {selected && <aside className="fixed right-0 top-0 bottom-0 w-full max-w-md bg-phantom-surface border-l border-phantom-border shadow-2xl z-30 overflow-auto"><div className="p-5 border-b border-phantom-border flex items-start justify-between"><div><div className="text-[10px] font-mono uppercase tracking-wider text-phantom-cyan">Asset detail</div><h2 className="text-lg font-semibold mt-1">{selected.host}</h2><div className="text-[11px] font-mono text-phantom-text-tertiary mt-1 break-all">{selected.target}</div></div><button onClick={() => setSelected(null)} className="p-2 text-phantom-text-tertiary hover:text-phantom-text-primary"><X size={16}/></button></div>
      <div className="p-5 space-y-5"><div className="grid grid-cols-2 gap-2"><Detail label="Status" value={selected.status}/><Detail label="Environment" value={selected.environment}/><Detail label="Type" value={selected.type}/><Detail label="Criticality" value={selected.criticality}/><Detail label="Risk" value={selected.risk}/><Detail label="Owner" value={selected.owner ?? 'Unassigned'}/></div>
        ...
        <div className="flex flex-wrap gap-2"><button onClick={() => nav(`/scans?target=${encodeURIComponent(selected.target)}`)} disabled={!can('scan:create')} className="h-9 px-3 rounded-lg bg-phantom-text-primary text-black text-xs font-semibold">Start assessment</button><button onClick={() => void resolve(selected)} className="h-9 px-3 rounded-lg border border-phantom-border text-xs flex items-center gap-2"><RefreshCw size={13}/>Resolve DNS</button><button onClick={() => openEdit(selected)} disabled={!can('workspace:manage')} className="h-9 px-3 rounded-lg border border-phantom-border text-xs flex items-center gap-2"><Pencil size={13}/>Edit</button></div>
      </div></aside>}
    ...
    </div><div className="px-5 py-4 border-t border-phantom-border flex justify-end gap-2"><button onClick={()=>setShowForm(false)} className="h-9 px-3 rounded-lg border border-phantom-border text-xs">Cancel</button><button onClick={()=>void save()} disabled={saving || !form.target.trim() || (editing ? !can('workspace:manage') : !can('scan:create'))} className="h-9 px-4 rounded-lg bg-phantom-text-primary text-black text-xs font-semibold disabled:opacity-40">{saving?'Saving…':editing?'Save changes':'Onboard asset'}</button></div></div></div>}
  </div>;
}
