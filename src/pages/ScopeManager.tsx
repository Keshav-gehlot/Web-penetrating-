...
export default function ScopeManager() {
  ...
  const load = async () => {
    ...
    try { const value = await getScope(); setScope(value); setDraft(toDraft(value)); }
  ...
  const add = (key: 'authorized_targets' | 'excluded_targets' | 'allowed_paths' | 'blocked_paths', value: string, clear: () => void) => {
    ...
    setDraft(current => ({ ...current, [key]: unique([...current[key], ...values]) })); clear(); setError('');
  };
  const remove = (key: 'authorized_targets' | 'excluded_targets' | 'allowed_paths' | 'blocked_paths', value: string) => {
    setDraft(current => ({ ...current, [key]: current[key].filter(x => x !== value) }));
  };
  const addPort = () => {
    ...
    setDraft(current => ({ ...current, allowed_ports: uniqueNumbers([...current.allowed_ports, port]).sort((a, b) => a - b) })); setPortInput(''); setError('');
  };
  const save = async () => {
    ...
    try { const value = await saveScope(draft); setScope(value); setDraft(toDraft(value)); setMessage('Scope policy saved.'); }
  ...
  return <div className="h-full overflow-auto bg-phantom-bg">
    <header className="sticky top-0 z-10 border-b border-phantom-border bg-phantom-bg/95 backdrop-blur px-6 py-5">
      <div className="max-w-6xl mx-auto flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-2"><Status enabled={draft.enabled} configured={scope.configured}/><button onClick={() => void load()} className="h-9 px-3 border border-phantom-border rounded-lg text-xs flex items-center gap-2 hover:bg-phantom-surface"><RefreshCw size={13}/>Refresh</button><button onClick={() => void save()} disabled={saving || !dirty} className="h-9 px-3 rounded-lg bg-phantom-text-primary text-black text-xs font-semibold flex items-center gap-2 disabled:opacity-40"><Save size={13}/>{saving ? 'Saving…' : 'Save policy'}</button></div>
      </div>
      {error && <div className="max-w-6xl mx-auto mt-4 rounded-lg border border-phantom-coral/30 bg-phantom-coral/10 px-3 py-2 text-xs text-phantom-coral">{error}</div>}
    ...
    <main className="max-w-6xl mx-auto p-6 space-y-5">
      <section className="grid lg:grid-cols-[1.5fr_1fr] gap-5">
        <Panel title="Authorization boundary" icon={LockKeyhole}>
          ...
          <label className="flex items-start gap-3 rounded-lg border border-phantom-border bg-phantom-panel p-3 cursor-pointer"><input type="checkbox" checked={draft.authorization_acknowledged} onChange={e => setDraft(d => ({...d, authorization_acknowledged: e.target.checked}))} className="mt-0.5 accent-cyan-400"/><span className="text-xs leading-5"><strong className="text-phantom-text-primary">I confirm this workspace has authorization to assess the configured targets.</strong><span className="block text-phantom-text-tertiary mt-0.5">Required before the scope can be enabled.</span></span></label>
          <label className="flex items-center justify-between rounded-lg border border-phantom-border bg-phantom-panel p-3"><span><span className="text-xs font-medium">Enforce workspace scope</span><span className="block text-[11px] text-phantom-text-tertiary mt-1">When disabled, scans cannot execute.</span></span><button onClick={() => setDraft(d => ({...d, enabled: !d.enabled}))} className={`w-11 h-6 rounded-full p-1 transition ${draft.enabled ? 'bg-phantom-cyan/70' : 'bg-phantom-surface border border-phantom-border'}`} aria-label="Toggle scope enforcement"><span className={`block w-4 h-4 rounded-full bg-white transition-transform ${draft.enabled ? 'translate-x-5' : ''}`}/></button></label>
        ...
        <Panel title="Policy limits" icon={ShieldCheck}>
          <NumberField label="Maximum requests / scan" value={draft.max_requests} min={1} max={250} onChange={v => setDraft(d => ({...d, max_requests: v}))}/>
          <NumberField label="Maximum concurrent scans" value={draft.max_concurrency} min={1} max={16} onChange={v => setDraft(d => ({...d, max_concurrency: v}))}/>
          <NumberField label="Maximum redirect hops" value={draft.max_redirects} min={0} max={5} onChange={v => setDraft(d => ({...d, max_redirects: v}))}/>
...
function Status({enabled,configured}:{enabled:boolean;configured:boolean}) { return <span className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[10px] uppercase tracking-wider ${enabled ? 'border-phantom-cyan/25 bg-phantom-cyan/5 text-phantom-cyan' : 'border-phantom-border bg-phantom-panel text-phantom-text-tertiary'}`}><span className={`w-1.5 h-1.5 rounded-full ${enabled ? 'bg-phantom-cyan' : 'bg-phantom-text-tertiary'}`}/>{enabled ? 'Enforced' : configured ? 'Configured / disabled' : 'Not configured'}</span>; }
