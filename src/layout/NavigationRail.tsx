...
type NavItemConfig = { icon: React.ComponentType<{ size?: number; strokeWidth?: number }>; label: string; path: string; permission?: Permission };

const NAV_ITEMS: NavItemConfig[] = [
  { icon: Activity, label: 'Overview', path: '/dashboard' },
  { icon: Server, label: 'Assets', path: '/assets', permission: 'scan:view' },
  ...
  { icon: Radar, label: 'OSINT', path: '/osint', permission: 'scan:view' },
  { icon: Radio, label: 'Net-Watch', path: '/netwatch', permission: 'scan:view' },
  { icon: Gauge, label: 'System Status', path: '/system-status', permission: 'scan:view' },
  { icon: FileText, label: 'Reports', path: '/reports', permission: 'report:export' },
];

const BOTTOM_NAV_ITEMS: NavItemConfig[] = [
  { icon: TerminalSquare, label: 'Terminal', path: '/terminal' },
  { icon: Users, label: 'Team', path: '/team', permission: 'users:manage' },
...
export function NavigationRail() {
  ...
  const visible = (item: NavItemConfig) => !item.permission || can(item.permission);

  return (
    <nav className="w-[68px] h-full flex flex-col items-center py-4 bg-phantom-bg flex-shrink-0 z-10">
      ...
      <div className="flex flex-col gap-2 w-full items-center mt-auto pt-4">
        {BOTTOM_NAV_ITEMS.filter(visible).map(item => <NavItem key={item.path} item={item} />)}
        <button onClick={() => { void logout().finally(() => nav('/login', { replace: true })); }} className="w-10 h-10 rounded-xl flex items-center justify-center text-phantom-text-tertiary hover:text-phantom-coral hover:bg-phantom-surface transition-all duration-200" title={`Sign out ${session?.actor ?? ''}`}>
          <LogOut size={18} />
        </button>
      </div>
    </nav>
...
function NavItem({ item }: { item: NavItemConfig; key?: React.Key }) {
  return (
    <NavLink to={item.path} className={({ isActive }) => cn('relative w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-200 group', isActive ? 'text-phantom-cyan bg-phantom-cyan/10' : 'text-phantom-text-secondary hover:text-phantom-text-primary hover:bg-phantom-surface')} title={item.label}>
      {({ isActive }) => <>
